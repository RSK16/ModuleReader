	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		button_qt.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View arg0) {
				...
				try {
					...
					{CustomPara.MemType =commfun.SortGroup(gr_qt_vw)==0?1:0;
					CustomPara.PersistType = commfun.SortGroup(gr_qt_sta)==0?1:0;
					CustomPara.RangeType = commfun.SortGroup(gr_qt_dic)==0?1:0;}
					CustomPara.AccessPwd = rpaswd;

					IMPINJM4QtResult CustomRet = myapp.Mreader.new IMPINJM4QtResult();
					myapp.Mreader.CustomCmd(myapp.Rparams.opant, CustomCmdType.IMPINJ_M4_Qt, CustomPara, CustomRet);
					 
				} catch (Exception e) {
					// TODO Auto-generated catch block
					Toast.makeText(SubQTActivity.this, MyApplication.Constr_excep+e.getMessage(),
	 ...
	 { 
		 ...
		 {  
			 ...
			 READER_ERR er=myapp.Mreader.ParamSet(Mtr_Param.MTR_PARAM_TAG_FILTER, g2tf_qt);
			 if(er==READER_ERR.MT_OK_ERR)
					Toast.makeText(SubQTActivity.this, MyApplication.Constr_ok+er.toString(),
							Toast.LENGTH_SHORT).show();
			 else
				 Toast.makeText(SubQTActivity.this, MyApplication.Constr_failed+er.toString(),
							Toast.LENGTH_SHORT).show();
		 }
		 else
		 { 
